class Solution:
    def leastInterval(self, tasks, n):
        ...
        while True:
            task_slot = sorted(
                [{'k':k, 'cnt': v} for k, v in task_dict.items() if v > 0],
                key=lambda x: x['cnt'],
                reverse=True
            )
            i = 0
            if len(task_slot) == 0:
                break
            while i <= n:
                if task_dict[task_slot[0]['k']] == 0:
                    break
                if i < len(task_slot) and task_slot[i]['cnt'] > 0:
                    task_dict[task_slot[i]['k']] -= 1
                interval += 1
                i += 1
...
if __name__ == '__main__':
    s = Solution()
    assert s.leastInterval(["A","A","A","A","A","A","B","C","D","E"], 2) == 16
    # assert s.leastInterval(["A","A","A","B","B","B", "C"], 2) == 8
    # assert s.leastInterval(["A","A","A","B","B","B"], 2) == 8
    # assert s.leastInterval(["A","B","C","D","E","A","B","C","D","E"], 4) == 10
